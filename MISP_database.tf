...
resource "aws_db_instance" "MISP_Database" {
  ...
  vpc_security_group_ids = ["${data.aws_security_group.MISP_node_ext.id}"]
...
resource "aws_db_subnet_group" "EKS_database_subnet" {
  ...
  subnet_ids = slice(tolist(data.aws_subnet_ids.MISP_subnets.ids), 0, length(data.aws_subnet_ids.MISP_subnets.ids) - 1)
  tags = {
